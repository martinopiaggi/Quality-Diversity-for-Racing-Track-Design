...
const mapelitePath = './src/utils/mapelite.xml'; 
...
function generateAndMoveTrackFiles() {
    ...
    executeCommand(`docker run -d -it ${dockerImageName}`, (containerId) => {
        ...
        // Step 2: Create the output directory inside the Docker container
        executeCommand(`docker exec ${containerId} mkdir -p /usr/share/games/torcs/tracks/dirt/output`, () => {

            // Step 3: Copy track files to Docker container
            executeCommand(`docker cp "output.xml" ${containerId}:/usr/share/games/torcs/tracks/dirt/output`, () => {

                // Step 4: Generate track files inside the Docker container 
